...
data_file1 <- file.path('ADFG','ADFG_Crab.csv')
data_file2 <- file.path('NMFS_76_91','NOAA_Crab.csv')
data_file3 <- file.path('NOAA_NBS','NOAA_RKC_NBS.csv')
...
#===============================================================================
# Data file name:  Keep in the data directory
adfg <- read.csv(file.path(data_dir,data_file1),na='.',header=TRUE)
# Add Sampling factor  
...
#===============================================================================
# Data file name:  Keep in the data directory
noaa <- read.csv(file.path(data_dir,data_file2),na='.',header=TRUE)
...
#===============================================================================
# Data file name:  Keep in the data directory
noaaNBS <- read.csv(file.path(data_dir,data_file3),na='',header=TRUE)
# 1: sublegal or female, 2: Legal
...
write.csv(crabdata,paste(data_dir,'crabdata_vast.csv',sep=''),row.names=T) 

#-------------------------------------------------------------------------------
nbs.st <- nbs.station
names(nbs.st)[3]  <- 'Area_NBS'
#  This extracts only Stations that both ADFG and NBS surveys were conducted 
crabdata <- merge(crabdata,nbs.st[,c(2,3)],by='ADFG_Station')
#-------------------------------------------------------------------------------
# USE Area_NBS to match areas each station represents
# calculate abundance 
crabdata$estkm <- with(crabdata,n*Area_NBS*(1.852^2)/Swept_km2)
crabdata$estnm <- with(crabdata,n*Area_NBS/Swept_NM2)
